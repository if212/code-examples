...
import {S1_HEADLINE_A, S1_HEADLINE_B, s1ActiveTile, s1Camera, s1Cursor, s1JitterFrame, s1MissFlare, s1MissP, s1MissShake, s1TileState} from './s1/state';
import {MissedSlotOnBoard} from './s1/MissedSlotMark';
import {MissFlare} from './s1/MissFlare';
 ...
 * ('again' stamp), then rushes tile 2 and skips 'Fix' (empty dashed slot) and moves on; at lf111 the slot
 * turns coral and the 'missed one' tag (settling at 1.2x = 18px) glitches on behind it with a coral flare. Headlines stay on smooth time.
    ...
    <AbsoluteFill>
      ...
      <Camera drift={cam}>
        ...
        <Board tile={(i) => s1TileState(i, f, fps)} jitterFrame={jf}>
          ...
          <MissFlare p={s1MissFlare(f)} jitterFrame={jf} shake={s1MissShake(f)} />
        </Board>
        <AbsoluteFill>
