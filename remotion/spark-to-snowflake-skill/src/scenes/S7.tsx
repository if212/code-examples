...
import {exitP, reveal} from '../motion';
 ...
 * board shrinks into the 'You' mini board while the pull-back reveals three teammate boards (still ember,
 * waiting). The skill folder rises at centre as a glowing hub ('shared in your repo'). Beams draw from the hub
 * to Leo, Priya and Sam; each teammate's cursor glides in, each board runs the same diagonal ember -> ice
 * wave and lands exactly one amber tile, which its owner then moves onto to review. Hold from lf80; the stage
 * recedes behind the S7 -> S8 light-wipe (Main) from lf95.
 */
...
export const S7: React.FC = () => {
  ...
  // S6's skill dock leaves as the camera pulls back
  const dockOut = exitP(f, T7.prevExit);
  ...
  return (
    <AbsoluteFill>
      ...
      {/* S6's dock, sliding out to the left */}
      {dockOut < 1 ? (
        <AbsoluteFill style={{pointerEvents: 'none'}}>
          <SkillDock lit={LIT_ALL} opacity={1 - dockOut} x={SKILL_DOCK.x0 - 48 * dockOut} />
        </AbsoluteFill>
      ) : null}

      {/* headlines: S6's line and sub-caption exit over lf0-10, then the S7 line */}
      {f < T7.prevExit + 12 ? <Headline text={COPY.s6} keyWord="traps" enterAt={-100} exitAt={T7.prevExit} /> : null}
      {f < T7.prevExit + 12 ? (
        <Caption text={COPY.s6Sub} enterAt={-100} exitAt={T7.prevExit} keyWord="NULLs" keyGradient="frost" />
      ) : null}
