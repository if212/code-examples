...
| A hidden rank edge for a source ELK drops | ELK puts a source just above its target, so of two sources the one with the shorter path sits a rank low, beside its peer's next node. A hidden edge from it to that node (`rules -> enrich: {class: dep; style.opacity: 0}`) lifts it into its peer's row (rules: y 145 -> 12, level with events). Label it like that node's edge in: bare beside a labelled one, it lands in the label's layer, a rank off. It costs width: that node centres between its two parents (+29 px here; +20 px a compare panel) |
...
| code | 8.4 px a character and 18.2 a line at 14px, never wrapped (break lines in the source); one width class on a spine of cards; a callee row as wide as its card; a before/after pair at full scale is exempt from W-aspect (playbooks/code.md) | 86 characters a line in one 800px card, 36 a side in a compare |
